...
local function step_process_one()
  ...
  local src_fh = io.open(source_file, "rb")
  if not src_fh then
    log_line(string.format("[WARN] %-28s  cannot open source file — skipping", tname))
    fh:close()
    process_idx = process_idx + 1
    return
  end
  ...
  -- Diagnostic: read first 12 raw bytes to verify data
  local raw_peek = src_fh:read(12)
  if raw_peek then
    local bytes_str = ""
    for i = 1, #raw_peek do
      bytes_str = bytes_str .. string.format("%02X ", raw_peek:byte(i))
    end
    log_line(string.format("     DIAG raw bytes at start: %s", bytes_str))
  end
  src_fh:seek("set", src_start_byte)
